...
function save_question($name, $title, $question, $question_id = null)
{
  $link = open_database_connection();
...
function add_answer($name, $answer, $question_id)
{
  ...
  $question_id = mysql_escape_string($question_id);
  $date = date("Y-m-d H:i:s");
  $query = "INSERT INTO hariult (id, answer, name, create_date, asuult_id, best)
...
function delete_answer($answer_id){
  ...
  $query = "DELETE FROM hariult WHERE id='$answer_id'";
  $result = mysql_query($query);
  close_database_connection($link);
}
...
function delete_answers($question_id){
  $link = open_database_connection();
  $question_id = mysql_escape_string($question_id);
  $query = "DELETE FROM hariult WHERE asuult_id='$question_id'";
  $result = mysql_query($query);
  close_database_connection($link);
}
?>
